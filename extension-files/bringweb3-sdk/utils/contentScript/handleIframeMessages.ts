...
const handleIframeMessages = ({ event, iframeEl, promptLogin }: Props) => {
    ...
    const { from, action, style, keyFrames, time, extensionId, url, domain, redirectUrl, iframeUrl, token, flowId } = event.data
    ...
    switch (action) {
        ...
        case ACTIONS.ACTIVATE:
            chrome.runtime.sendMessage({ action, from: "bringweb3", domain, extensionId, time, redirectUrl, iframeUrl, token, flowId })
            break;
